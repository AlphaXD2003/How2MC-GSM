...
const Servers = () => {
    ...
    const getUsersServer = async () => {
        const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/servers/user/servers`, {
    ...
    const getServerDetails = async (serverId: Number) => {
        ...
        const serverInfo = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/servers/details/${sid}`, {
            withCredentials: true
        })
    ...
    const deleteServer = async (serverId: Number) => {
        try {
            const response = await axios.delete(`${import.meta.env.VITE_BACKEND_URL}/servers/delete/safe/${serverId}`,{
                withCredentials: true
            }
        )
                        ...
                        servers ?
                            servers.map((server: any, index: Number) => (
                                <TableRow key={server.server_id || index}>
                                    <TableCell className="font-medium">{server.server_id}</TableCell>
                                    ...
                                    <TableCell className="w-[100px]">
                                        <Modal>
                                            <ModalTrigger className="bg-black dark:bg-white dark:text-black text-white flex justify-center group/modal-btn">
                                                <span onClick={() => getServerDetails(server.server_id)} className="group-hover/modal-btn:translate-x-40 text-center transition duration-500">
                                                    DETAILS
                                                </span>
                                                ...
                                                <ModalContent>
                                                    ...
                                                    <div className='text-xl'>
                                                        ...
                                                        <div className='bg-[var(--semi-dark2)] p-2 rounded-lg'>
                                                            ...
                                                            <div className='mb-2'>
                                                                Server Type: {server.serverInfo.status.toUpperCase()}
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div className='mb-2 mt-6'>
                                                            <div className='underline font-bold'>
                                                                ...
                                                                Object.entries(server.serverInfo).map(([key, value]: any, index: Number) => {
                                                                    if (key === 'lastRenewed') {
                                                                        ...
                                                                        return <div key={key}>
                                                                            <div className='mb-2 flex gap-2'>
                                                                                ...
                                                                                <div>{value.toLocaleString().toUpperCase()}</div>
                                                                            </div>
                                                                        </div>;
                                                                    }
                                                                })
                                    ...
                                    <TableCell className="w-[100px]">
                                        <Button
                                            onClick={() => navigate(`/dashboard/servers/${server.server_id}`)}
                                            variant="contained" color="warning" >OPEN</Button>
                                    </TableCell>
